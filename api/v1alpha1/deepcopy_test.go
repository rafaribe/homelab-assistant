...
func TestVolSyncUnlockStatus_DeepCopy(t *testing.T) {
	startTime := metav1.Now()
	completionTime := metav1.Now()

	original := &VolSyncUnlockStatus{
	...
	if copied.Phase != original.Phase {
		t.Error("DeepCopy failed: Phase mismatch")
	}

	if copied.JobName != original.JobName {
		t.Error("DeepCopy failed: JobName mismatch")
	}

	if copied.StartTime == nil || *copied.StartTime != *original.StartTime {
		t.Error("DeepCopy failed: StartTime mismatch")
	}

	if copied.CompletionTime == nil || *copied.CompletionTime != *original.CompletionTime {
		t.Error("DeepCopy failed: CompletionTime mismatch")
	}
}

func TestVolSyncUnlockSpec_DeepCopy(t *testing.T) {
	ttl := int32(3600)
	original := &VolSyncUnlockSpec{
		AppName:                 "test-app",
		Namespace:               "test-ns",
		ObjectName:              "test-obj",
	...
	if copied.TTLSecondsAfterFinished == nil || *copied.TTLSecondsAfterFinished != *original.TTLSecondsAfterFinished {
		t.Error("DeepCopy failed: TTLSecondsAfterFinished mismatch")
	}

	// Modify original to ensure deep copy
	*original.TTLSecondsAfterFinished = 7200
	if *copied.TTLSecondsAfterFinished == 7200 {
		t.Error("DeepCopy failed: TTLSecondsAfterFinished was not deeply copied")
	}
}
